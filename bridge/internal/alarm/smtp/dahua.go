...
func NewDahuaParser(logger *logger.Logger) *DahuaParser {
	return &DahuaParser{
		logger: logger,
	}
}
...
func (d *DahuaParser) ParseAlarm(data interface{}) (*database.PublicAlarmInsert, error) {
	email, ok := data.(*smtp.Mail)
	if !ok {
	...
	if dahuaData.AlarmName == "" {
		d.logger.Log(logger.Error, "Missing required field: Alarm Event")
		return nil, fmt.Errorf("missing required field: Alarm Event")
	}

	d.logger.Log(logger.Info, "Parsed Dahua alarm event: %s from device %s", dahuaData.AlarmName, dahuaData.CameraId)

	// Convert LegacyType to Event
	event := &database.PublicAlarmInsert{
